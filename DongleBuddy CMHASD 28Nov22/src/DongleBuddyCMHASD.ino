#define VERSION "\nDongleBuddy CMHASD V0.221128 by keith.rickard@hotmail.com"  // Started Jun 2022
#include <Arduino.h>
...
//  return;
  ...
  if ((analogRead(RS232_12V) < 100) || (analogRead(GUIDE_12V) >100)) {                    // Wrong plug in wrong socket?
    ...
    while((analogRead(RS232_12V) < 100) || (analogRead(GUIDE_12V) > 100)) {               // Restart when plugs changed
      if (millis() > timerLED) {
        timerLED = millis() + 100;
        led[0] ^=1;
        for (byte i = 1; i <= 4; i++) digitalWrite(led[i], 0);
  ...
  if (millis() > timerLED) {
    ...
    else if (!gbReady) for (byte i = 1; i <=4; i++) digitalWrite(led[i], (led[0] == 0) && mode == MODE_PC && i == 1); // Dir LEDs flashing - GuideBuddy not working
...
void checkGuideBuddy() {
  ...
  Serial1.print(":Qe#:RG#:A"); Serial1.print(alignMode[0]); Serial1.print('#'); // Stop any RA move and put back in original mode
...
void guideBuddy() {
  ...
  if (Serial.readBytes(buf, 1)) {
     switch (buf[0]) {
      ...
      case 'V': Serial.println(VERSION); buf[0] = 0; break;
...
void getAzimuth() {
  ...
  do {
    ...
    //if (a >= 21600) break;                                        // Return if outside range - assume connection is lost
    ...
    if (debug) {
      Serial.print(lastAZM); Serial.print(" v "); Serial.println(tmpAZM);
    }
  ...
  if (SerialMix.read() == '$') {
    ...
    if (line.startsWith("GNRMC")) {
      ...
      if (debug) {
        ...
        Serial.println(gpsData.lat, 4);
        Serial.print("Lng:  "); Serial.println(gpsLng);
        Serial.println(gpsData.lng, 4);
...
bool validate(String sentence) {
  for (byte i = 0; i < sentence.length(); i++) {
    if (sentence.charAt(i) == '$') return 0;
  }
  for (byte i = 0; i < sentence.length(); i++) {
    if (sentence.charAt(i) == '*') return 1;
  }
  return 0;
}
// --------------------------------------------------------------------------------------------------
...
void sendDate() {
  ...
  mth = gpsData.day;
  yr = gpsData.yr + 2000;
  //gpsData.fix |= B0010;                   // Get GPS time again
  ...
  if (buf[0] == '0') {
    gpsData.fix |= B0010;
    return;                               // Quit if date is invalid
  }
  clearBuf();
...
void sendLoc() {
  ...
  lat = abs(lat);                                           // Add 0.5 arcmin to account for rounding
  lat += (lat - int(lat)) >= 0.5;                           // Rounding adj  
  deg = int(lat / 100);
  min = int(lat - deg *100);
...
// The GPS data longitude is negative west of Greenwich (0 to -180) and postive east (0 to 180)
  sign = lng >= 0 ? '+' : '-';  
  lng = abs(lng);
  lng += (lng - int(lng)) >= 0.5;                           // Rounding adj  
  deg = int(lng / 100);
  min = int(lng - deg * 100);
  if (gpsData.lng >= 0) {
    if (deg || min) deg = 360 - deg - (min > 0);
    if (min) min = 60 - min;
  } else {
    deg = -deg;
    min = -min;
