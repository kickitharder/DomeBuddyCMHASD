#define VERSION "\nDomeBuddy Controller CMHASD V2.220304 by keithrickard@hotmail.com"
#define LCD_VERS "$$Version 2.230304$Nby Keith Rickard"
#include <Arduino.h>
...
void aConfirmSafety() {
  domeSafe = 0;
  LCD("$$Are all clips   $Nundone?       OK"); sendDisp();
  if (waitOKesc(0) & btnE) return;
...
byte manualMove(byte mtr) { // return
  ...
  while(1) {                                          // Main loop
    ...
    do {
      ...
      if (button & btnE) {                            // Esc button pressed?
        while(scanBtns());
        delay(500);
        while(scanBtns());
        return btnE;                                  // Exit
      }
      if ((button & (btnO | btnH)) && !slaved) sync2scope();
