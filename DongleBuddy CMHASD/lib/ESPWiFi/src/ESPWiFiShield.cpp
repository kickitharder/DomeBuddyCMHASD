/******************************************************************************
Modified by Keith Rickard 29 Oct 2022
...
Modified by Keith Rickard 20 Jun 2018, 12 Sep 2022
******************************************************************************/

#include "ESPWiFiShield.h"
...
bool ESP8266Class::begin(HardwareSerial &hSerial, long baudRate)
{
...
bool ESP8266Class::begin(uint8_t rx_pin, uint8_t tx_pin, long baudRate)
{
...
void ESP8266Class::sendCommand(const char * cmd, uint8_t type, const char * params)
{
	...
	if (DEBUG) {
		Serial.println("\nSending: ");
		Serial.write(0x41);  Serial.write(0x54);
		Serial.print((const __FlashStringHelper*)cmd);
	}
	_serial->write(0x41);_serial->write(0x54);
