#define VERSION "\nDongleBuddy CMHASD V0.221101 by keith.rickard@hotmail.com"  // Started Jun 2022
#include <Arduino.h>
...
#define   ENQ_PERIOD    5000    // Time in ms between send DB controller azimuth info
...
void setup() {
  ...
  if (!digitalRead(BTN_PC)) {
    while (!digitalRead(BTN_PC)) delay(20);
    debug = 1;
  }
  ...
  if (!digitalRead(BTN_BT) && digitalRead(BTN_PC) && !digitalRead(BTN_WF)) debug = 1;
  mode = MODE_NUL;
  ...
  delay(100);
  Serial3.end();                                                                // Clear receive buffer
...
void checkDevice() {
  ...
  if (mode == MODE_NUL) {                                       // No connection yet
...
// etc, can be delivered to the LX200 mount via its control panel's CCD port.                                        */
  ...
  unsigned long timer = millis() + ENQ_PERIOD;    // Send data to DB controller every 5 seconds
  timerEvent = millis();
  while (millis() < timer) {
    checkDevice();
    if (mode) {
...
// --------------------------------------------------------------------------------------------------
void wfComms() {
  if (debug) Serial.println("{wfComms2}");
  ...
  while (client.connected()) {
    if (client.available()) {
      delay(10);
      cmdStr = "";
      wifiStr = client.readStringUntil('#') + '#';
      if(wifiStr == "0,CLOSED\r\n#") {
        Serial.println("Wifi connection has been closed");
        return;
      }
      for (byte i = wifiStr.length() - 1; i >= 0; i--) {                  // Search backwards from end of string for a colon
        ...
        if (wifiStr.charAt(i) == ':') if (!isDigit(wifiStr.charAt(i + 1))) break;
      }

      if (cmdStr.charAt(0) == ':') {
        Serial.print("[W>T]"); Serial.println(cmdStr);
        ...
        if (type) {                                                       // Get the LX200 response and deal with it correctly
          ...
          Serial.print("[T>W]"); Serial.println(respStr);
...
bool pcComms() {
  ...
  if (Serial.available()) {                                         // PC command
    ...
    if (strchr("<>v^_|%?=~!@", Serial.peek())) guideBuddy();        // Action any GuideBuddy commands
    else {
...
bool btComms() {
  ...
  while(Serial2.available()) {                                      // PC command
    analogWrite(led[mode], LED_OFF);                                // Comms in progress
    timerEvent = millis() + EVENTDELAY;
    if (strchr("<>v^_|%?=~!@", Serial2.peek())) guideBuddy();       // Action any GuideBuddy commands
    else {
      lxCmd[0] = lxCmd[1]; lxCmd[1] = lxCmd[2];                     // If :SC command has been received to 
...
// If comms between telescope and device occur then the device LED will flash.
// If comms between telescope and DomeBuddy occur then the DomeBuddy LED will flash.
// Applicable directional LEDs will light if a guide pulse occurs and turn off when done.
...
//  return;
  ...
  if ((analogRead(RS232_12V) < 100) || (analogRead(GUIDE_12V) >100)) {                    // Wrong plug in wrong socket?
    ...
    while((analogRead(RS232_12V) < 100) || (analogRead(GUIDE_12V) > 100)) {               // Restart when plugs changed
      if (millis() > timerLED) {
        timerLED = millis() + 100;
        led[0] ^=1;
        for (byte i = 5; i <= 8; i++) i > 4 ? analogWrite(led[i], led[0] * LED_BRIGHT) : digitalWrite(led[i], led[0]);
  ...
  if (millis() > timerLED) {
    timerLED = millis() + 25;
    ++led[0] &= B1111;
    for (byte i = 5; i <= 7; i++) analogWrite(led[mode ? mode : i], scopeConnected ? 64 : ((led[0]/8) & B01 ? LED_BRIGHT : 0)); // Device LED
    if (!ESPworking) analogWrite(led[MODE_WF], 0);                                                                 // ESP not working?
    if (gettingGPS) for (byte i = 1; i <= 4; i++) digitalWrite(led[i], i == led[0]/4 + 1);                         // Direction LEDs - GPS fixing
    else if (!gbReady) for (byte i = 1; i <=4; i++) digitalWrite(led[i], led[0] == 0);                             // GuideBuudy not working
    analogWrite(LED_DB, dbCxn() ? LED_DIM : ((led[0]/4) & B01 ? LED_DIM : LED_OFF));                               // DomeBuddy LED
  }
  if (!gettingGPS && gbReady) if (!movingEW && !movingNS) for (byte i = 1; i <= 4; i++) digitalWrite(led[i], LOW); // Direction LEDS
}
// --------------------------------------------------------------------------------------------------
bool dbCxn() {
//  return 1;
...
void checkGuideBuddy() {
  ...
  do {
    ...
    Serial1.print("#:Q#:Qn#:Qs#:Qe#:Qw#:AP#:GD#");              // Stop any motion, put into POLAR mode & get Dec
    pinMode(NORTH, INPUT_PULLUP);
    pinMode(SOUTH, INPUT_PULLUP);
    pinMode(EAST, INPUT_PULLUP);
...
void guideBuddy() {
  ...
  if (Serial.readBytes(buf, 1)) {
     switch (buf[0]) {
      ...
      case '?': buf[0] = gbReady + '0';  break; // Return '!' to indicate GuideBuddy is responding
...
void moveNS(byte direction){          // Direction is NORTH or SOUTH
  ...
  digitalWrite(direction, LOW);       // Ground the line to send a pulse
  digitalWrite((direction == NORTH ? LED_S : LED_N), LOW);
  digitalWrite((direction == NORTH ? LED_N : LED_S), HIGH);
...
// LOOK FOR THE MEADE LX200
  ...
  delay (200);
  if (Serial1.peek() >= 0) {
    if (!Serial1.readBytes(buf, 9)) return;
    if (buf[8] == '#') {
...
bool testLXlink () {
  ...
  Serial1.print(":\x06#");                                // Send a short command for a 1 byte response for speed to test cxn
  ...
  else if (debug) Serial.println("LX200 found");
...
void sendAZM(){
  ...
  if (!dbCxn()) return;
// Sends ':', 'S' or 'T', Xh, Xl, ~Xh, ~Xl, '#', e.g. for lxAZM  = 0, ":T00FF#" is sent
// lxSLEW is 'S' or 'T' and lxAZM is the scope's azimuth 0-255 (0 is due north)
  ...
  if (debug) {
    ...
    Serial.print(lxSLEW);                           // Send slewing info ('S' = slewing, 'T' = tracking, '')
  ...
  SerialMix.print(lxSLEW);                          // Send slewing info ('S' = slewing, 'T' = tracking)
  if (lxAZM < 16) SerialMix.print('0');             // Send Azimuth as two hexdigit number
  SerialMix.print(lxAZM, HEX);                      // Send only the HSB (0 - 255) i.e. 256 point resolution
    ...
    if (line.startsWith("GNRMC")) {
      ...
      if (gpsData.fix & B0001){                                 // Get GPS time
        ...
        if (gpsTime != "") {
          ...
          gpsData.fix &= B1110;
          Serial.print("Time: "); Serial.println(gpsData.fix);
          sendTime();
          sendSidTime();
        }
      }
      if (gpsData.fix & B0010) {                                // Get GPS date
        ...
        if (gpsDate != "") {
          ...
          gpsData.fix &= B1101;
          Serial.print("Date: "); Serial.println(gpsData.fix);
          sendDate();
        }
      }
...
// Longitude for the LX200 is b from 0 deg 0 min to 359 deg 59 min, increasing west from Greenwich.
