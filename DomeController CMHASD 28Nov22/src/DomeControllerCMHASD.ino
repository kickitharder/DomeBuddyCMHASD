#define VERSION "\nDomeBuddy Controller CMHASD V2.221128 by keithrickard@hotmail.com"
#define LCD_VERS "$$Version 2.221128$Nby Keith Rickard"
#include <Arduino.h>
...
void aTrackTelescope() {
  ...
  while (slaved == 1) {
    ...
    if (trace) {
      trace = trace == 255 ? 1 : trace + 1;
      Serial.print("trace: "); Serial.println(trace);
    }
...
void setupISR() {   // Mode = 0 live, Mode = 1 test rig
  ...
  if (testMode) return;
  if (ee.encDir) attachInterrupt(digitalPinToInterrupt(ENC_ChA), encISRcw, CHANGE);
