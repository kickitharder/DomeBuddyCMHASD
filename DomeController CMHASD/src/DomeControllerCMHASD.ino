#define VERSION "\nDomeBuddy Controller CMHASD V2.221101 by keithrickard@hotmail.com"
#define LCD_VERS "$$Version 2.221101$Nby Keith Rickard"
#include <Arduino.h>
...
#define PERIOD_CXN    10000   // Timeout period for not receiving comms from the dongle
#define BTBAUD        38400

#define ENC_ChA       21  // W  (SCL pin)
#define ENC_ChB       20  // Bk (SDA pin)
#define HOME_SW       19  // Gy Homeswitch
...
void setup() {
  ...
  scopeAZM = domeAZM = enc2azm(currEnc);
  lcdUDGs();
  do {
    ...
    if (waitBtns(1500)) break;
    LCD("$$Martin Crow$NSteve Floodgate");
...
//    LCDintAt(1, 3, int(360.0 * (float(currEnc) / ee.enc360)), 3); LCD("` ");  // Azimuth in degrees
...
void aParkDome() {
  if (domeAZM != ee.homeAZM) {
    ...
    slaved = parking = 1;
    while (parking && moveDome(ee.homeAZM));
    if (!parking) return;
  }
  LCD ("$$DOME PARKED$");
...
void aSetHomePosn() {
  ...
  LCD("$$Home position$Nhas be set.   OK");
  sendDisp();
...
void bSettings() {
  ...
  do{
    ...
    switch(selection & 15){
      ...
      case 2: eBuzzer();            break;
      case 3: bLCDTimeout();        break;
      case 4: cMotorSetup();        break;
      case 5: bEncoderDirection();  break;
      case 6: bAzimuthMode();       break;
      case 7: gCheckHardware();     break;
      case 8: bSetPassword();       break;
      case 9: bFactoryReset();     break;
      case 10: return;              break;
      case 11: bShowVars();         break;
    }
  } while (selection & 15);
}
//--------------------------------------------------------------------------------------------------------------
...
void bShowVars2() {
  ...
  do {
    ...
    selection = menu(items, 14, selection);
    switch(selection & 15){
      case 13: bShowVars3();  break;
...
void cMotorSetup() {
  ...
  do{
    ...
    items[9][13] =  ee.dir1cw + '0';
    items[10][13] = ee.dir2cw + '0';
    items[11][13] = ee.dir3cw + '0';
    selection = menu(items, 13, selection);
    switch(selection & 15){
      case 1: cAlignMotors(1);  break;
      case 2: cAlignMotors(2);  break;
      case 3: cAlignMotors(3);  break;
      case 4: dMaxPWMs();       break;
      case 5: dStartPWMs();     break;
      case 6: dStopPWMs();      break;
      case 7: cRampSpeed();     break;
      case 8: cStopSpeed();     break;
      case 9: cStuckTime();     break;
      case 10: cDirMotor(1);    break;
      case 11: cDirMotor(2);    break;
      case 12: cDirMotor(3);    break;
...
void dMaxPWMs() {
  ...
  do {
    embedValue(items[0], ee.maxPWM1cw);
    embedValue(items[1], ee.maxPWM1ccw);
    embedValue(items[2], ee.maxPWM2cw);
    embedValue(items[3], ee.maxPWM2ccw);
    embedValue(items[4], ee.maxPWM3cw);
    embedValue(items[5], ee.maxPWM3ccw);
    selection =  menu(items, 7, selection);
    switch (selection & 15) {
      case 1: ee.maxPWM1cw = setPWM(1, 1, ee.maxPWM1cw, (ee.startPWM1cw > ee.stopPWM1cw) ? ee.startPWM1cw : ee.stopPWM1cw, 255); break;
      case 2: ee.maxPWM1ccw = setPWM(2, 1, ee.maxPWM1ccw, (ee.startPWM1ccw > ee.stopPWM1ccw) ? ee.startPWM1ccw : ee.stopPWM1ccw, 255); break;
      case 3: ee.maxPWM2cw = setPWM(1, 2, ee.maxPWM2cw, (ee.startPWM2cw > ee.stopPWM2cw) ? ee.startPWM2cw : ee.stopPWM2cw, 255); break;
      case 4: ee.maxPWM2ccw = setPWM(2, 2, ee.maxPWM2ccw, (ee.startPWM2ccw > ee.stopPWM2ccw) ? ee.startPWM2ccw : ee.stopPWM2ccw, 255); break;
      case 5: ee.maxPWM3cw = setPWM(1, 3, ee.maxPWM3cw, (ee.startPWM3cw > ee.stopPWM3cw) ? ee.startPWM3cw : ee.stopPWM3cw, 255); break;
      case 6: ee.maxPWM3ccw = setPWM(2, 3, ee.maxPWM3ccw, (ee.startPWM3ccw > ee.stopPWM3ccw) ? ee.startPWM3ccw : ee.stopPWM3ccw, 255); break;
      case 7: return; break;
    }
  } while (selection & 15);
  EEPROM.put(0,ee);
...
void dStartPWMs() {
  ...
  do {
    embedValue(items[0], ee.startPWM1cw);
    embedValue(items[1], ee.startPWM1ccw);
    embedValue(items[2], ee.startPWM2cw);
    embedValue(items[3], ee.startPWM2ccw);
    embedValue(items[4], ee.startPWM3cw);
    embedValue(items[5], ee.startPWM3ccw);
    selection =  menu(items, 7, selection);
    switch (selection & 15) {
      case 1: ee.startPWM1cw = setPWM(3, 1, ee.startPWM1cw, 0, ee.maxPWM1cw); break;
      case 2: ee.startPWM1ccw = setPWM(4, 1, ee.startPWM1ccw, 0, ee.maxPWM1ccw); break;
      case 3: ee.startPWM2cw = setPWM(3, 2, ee.startPWM2cw, 0, ee.maxPWM2cw); break;
      case 4: ee.startPWM2ccw = setPWM(4, 2, ee.startPWM2ccw, 0, ee.maxPWM2ccw); break;
      case 5: ee.startPWM3cw = setPWM(3, 3, ee.startPWM3cw, 0, ee.maxPWM3cw); break;
      case 6: ee.startPWM3ccw = setPWM(4, 3, ee.startPWM3ccw, 0, ee.maxPWM3ccw); break;
      case 7: return; break;
    }
  } while (selection & 15);
  EEPROM.put(0,ee);
...
void dStopPWMs() {
  ...
  byte selection = 1;
  do {
    embedValue(items[0], ee.stopPWM1cw);
    embedValue(items[1], ee.stopPWM1ccw);
    embedValue(items[2], ee.stopPWM2cw);
    embedValue(items[3], ee.stopPWM2ccw);
    embedValue(items[4], ee.stopPWM3cw);
    embedValue(items[5], ee.stopPWM3ccw);
    selection =  menu(items, 7, selection);
    switch (selection & 15) {
      case 1: ee.stopPWM1cw = setPWM(5, 1, ee.stopPWM1cw, 0, ee.maxPWM1cw); break;
      case 2: ee.stopPWM1ccw = setPWM(6, 1, ee.stopPWM1ccw, 0, ee.maxPWM1ccw); break;
      case 3: ee.stopPWM2cw = setPWM(5, 2, ee.stopPWM2cw, 0, ee.maxPWM2cw); break;
      case 4: ee.stopPWM2ccw = setPWM(6, 2, ee.stopPWM2ccw, 0, ee.maxPWM2ccw); break;
      case 5: ee.stopPWM3cw = setPWM(5, 3, ee.stopPWM3cw, 0, ee.maxPWM3cw); break;
      case 6: ee.stopPWM3ccw = setPWM(6, 3, ee.stopPWM3ccw, 0, ee.maxPWM3ccw); break;
      case 7: return; break;
    }
  } while (selection & 15);
  EEPROM.put(0,ee);
...
void fSetResolution() {
  ...
  calibrating = 1;
  long value = getValue(ee.enc360, 99999L, MIN_RES, 1, 4); 
  if (value == ee.enc360) return;
  ee.enc360 = value;
}
//--------------------------------------------------------------------------------------------------------------
void fAutoCalib() {
  LCD("$$Auto calibration$NNot implemented!");
  sendDisp();
  waitOKesc(0);
}
...
//--------------------------------------------------------------------------------------------------------------
void gHomeSwitch() {
  int lastHomeSw = 2;                    // '2' forces display to be sent to PC on first pass
  LCD("$$Home switch test");
  LCD("$NStatus:");
  do {
    if (lastHomeSw != digitalRead(HOME_SW)) {
      lastHomeSw = digitalRead(HOME_SW);
      LCDintAt(1, 8, digitalRead(HOME_SW), 0);
      lastHomeSw ? LCD(" OPEN  ") : LCD(" CLOSED");
      sendDisp();
    }
    scanBtns();
  } while (!(btnsLast & btnO) && !(btnsLast & btnE));
...
//focus => bits 3-0: item no. in focus (to be arrowed) bits 7-4: item to show on top of LCD display
  ...
  do{
    ...
    sendDisp();
    while (!scanBtns()) {       
      LCDat(1, 15);                                                                  // Wait for a button press
      btCxn() ? LCD("^") : LCD(" ");
      sendDisp();
    }
    beepBtns();
...
//--------------------------------------------------------------------------------------------------------------
byte moveDome(byte azm) {                       // Move dome to target azimuth
  targAZM = azm;
  ...
  sendDisp();
  for (byte i; i <= 3; i++) {                   // Give audible warning of move about to take place
    beep(50, ee.bzMoves);
    delay(50);
  }
  ...
  do {                                          // Ramp-up the motors
    ...
    if (pressedBtns()) {slaved = 2; break;}     // Stop moving and slaving immediately if a button is pressed
  ...
  while (--pwm) {                                                     // Slow down the motors
    if (slaved != 1) {
      delay(ee.stopSpeed);                                            // If slew aborted (slaved = 0) then just slow down
  ...
  // FINISH-UP -------------------------------------------------------------------------------------------------
  analogWrite(PWM1, 0);                     // Switch off the motors
  analogWrite(PWM2, 0);
  analogWrite(PWM3, 0);
  ...
  if (!slaved) LCD("$HDOME IS STUCK$F");
  if (slaved == 2) parking ? LCD("$HPARKING ABORTED") : LCD("$HTRACKING ABORTED");
  if (slaved != 1) {
    LCD("$NPress OK or ESC ");
    sendDisp();
    beep(1000, ee.bzAlert);
    if (!slaved) beep(1000, ee.bzAlert);   // Dome is stuck - beep continuously
    waitOKesc(0);
    beep(1, ee.bzAlert);
    parking = slaved = 0;                  // Stop reacting to received azimuth from scope
  }
  cxnTimer = millis() + PERIOD_CXN;
  degTimer = millis() + 3000;
  if (slaved == 2) return slaved;           // A button was press so stop slaving
...
void sendInfo(char msg[]) {
  ...
  Serial.print("currEnc:  "); Serial.println(prevEnc);
  ...
  Serial.print("dir:      "); Serial.print(dir); Serial.print(" "); Serial.println(dir ? '>' : '<');
...
byte manualMove(byte mtr) { // return
  ...
  if (slaved) {
    ...
    LCD("$HPress OK if done");
  } else {
    LCD("$$Press < or > to$Nrotate the dome");
...
//   0123456789ABCDEF
    ...
    do {                                                      // Ramp-up the motors
      ...
      if (mtr == 1 || !mtr) analogWrite(PWM1, map(pwm, 0, 255, startPWM1, mtr ? 70 : maxPWM1));
      if (mtr == 2 || !mtr) analogWrite(PWM2, map(pwm, 0, 255, startPWM2, mtr ? 70 : maxPWM2));
      if (mtr == 3 || !mtr) analogWrite(PWM3, map(pwm, 0, 255, startPWM3, mtr ? 70 : maxPWM3));
    } while(++pwm);                                           // Loop if maximum speed not reached
    ...
    do {
      pwm--;
      if (mtr == 1 || !mtr) analogWrite(PWM1, map(pwm, 0, 255, stopPWM1, mtr ? 70 : maxPWM1));
      if (mtr == 2 || !mtr) analogWrite(PWM2, map(pwm, 0, 255, stopPWM2, mtr ? 70 : maxPWM2));
      if (mtr == 3 || !mtr) analogWrite(PWM3, map(pwm, 0, 255, stopPWM3, mtr ? 70 : maxPWM3));
      delay(ee.stopSpeed);
      LCDintAt(1, 4, pwm, 3);
      if (!slaved) {
        LCDintAt(0, 9, calibrating ? currEnc :normEnc(), 5);            // Keep the dome turning while button is being pressed
...
bool getScopeAZM() {                                            // Get and interpret scope's azimuth from DomeBuddy Dongle via Bluetooth
  ...
  switch (buf[1]) {
    case 'T' :  
      byte value, chksum;                                           // Telescope is tracking
      newData = 0;
      ...
      if (value == byte(~chksum)) {                                 // Is the data valid?
        ...
        LCD("Scope azmth:"); LCDint((360UL * byte(scopeAZM + ee.azmAdj) / 256.0)); LCD("`");
        newData = 1;
      }
...
bool getFromUSB() {                                           // Returns 1 for "key pressed", 0 if not
  ...
  switch (Serial.read()) {
    ...
    case 'B': btStat = 2; return 0; break;                      // Simulate Bluetooth connection
    case 'b': btStat = 1; return 0; break;                      // Simulate no Bluetooth conneciton
    case 'X': btStat = 0; return 0; break;                      // Use real Bluetooth connection status
    case 'x': btStat = 0; return 0; break;                      // Use real Bluetooth connection status
...
void sendVars() {
  ...
  Serial.print("ee.encDir    \t"); Serial.print(ee.encDir); Serial.println(ee.encDir ? " >CW (rig)" :  " <CCW");
...
void setupISR() {   // Mode = 0 live, Mode = 1 test rig
  ...
  if (ee.encDir) attachInterrupt(digitalPinToInterrupt(ENC_ChA), encISRlive, CHANGE);  // For test rig
  else attachInterrupt(digitalPinToInterrupt(ENC_ChA), encISRtest, CHANGE);             // For live mode
}
//--------------------------------------------------------------------------------------------------------------
void encISRtest() {                 // Clockwise (probably for live mode)
...
//--------------------------------------------------------------------------------------------------------------
void encISRlive() {                 // Counter clockwise (for test rig)
  switch (PIND & B11) {             // PIND Bit 1 is ChA, PIND Bit 0 is ChB
    case B00: currEnc++; return;
    case B01: currEnc--; return;
    case B10: currEnc--; return;
